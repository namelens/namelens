...
v0.2.1 makes NameLens agent-ready: a headless server API for remote
integration, a guided setup wizard for fast onboarding, and safety guardrails
that prevent false confidence from availability-only results.
...
Anthropic excels at structured reasoning, conflict awareness, and deep
technical analysis — particularly useful for `generate` and `review` workflows.
See [Workflows & Best Practices](../user-guide/workflows.md) for provider
selection guidance.
...
Docker: `ghcr.io/namelens/namelens:v0.2.1`
